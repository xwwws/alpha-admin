      ...
      render: (text, record) => [
        ...
        <Tooltip placement="top" title="添加" key={'add'}>
          <Button
            ...
            onClick={() => navigate(`/account/menus/create/${record.id}`)}
          ></Button>
        </Tooltip>,
        <Tooltip placement="top" title="编辑" key={'edit'}>
          <Button
            type={'link'}
            icon={<EditOutlined />}
            onClick={() => navigate(`/account/menus/edit/${record.id}`)}
          ></Button>
        </Tooltip>,
        <Tooltip placement="top" title="分配权限" key={'assignPermissions'}>
          <Button
            type={'link'}
            icon={<ToolOutlined/>}
            onClick={() => navigate(`/account/menus/${record.id}/assign-permissions`)}
          ></Button>
        </Tooltip>,
        <Popconfirm
          key="del"
    ...
    <PageContainer
      extra={[
        <Button
          ...
          icon={<PlusOutlined/>}
          type={'primary'}
          onClick={() => navigate('/account/menus/create/0')}
        >
          添加顶级菜单
        </Button>,
      ]}
